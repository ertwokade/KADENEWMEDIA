...
export default async function handler(req, res) {
  ...
  if (action === 'reply' && req.method === 'POST') {
    try {
      ...
      const mailSubject = cleanHeader(subject || `Re: Kade New Media — ${message.service && message.service !== '-' ? message.service : 'İletişim'}`);

      await transporter.sendMail({
        from: `"Kade New Media" <${smtpUser}>`,
        to: message.email,
        subject: mailSubject,
        html: `
          <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px;background:#0a0a0a;color:#fff;border-radius:12px;">
            <div style="text-align:center;padding:16px 0;border-bottom:1px solid #333;">
              <h2 style="color:#eac321;margin:0;">⚡ Kade New Media</h2>
            </div>
            <div style="padding:24px 20px;">
              ...
              <p style="color:#888;font-size:12px;margin:0;">Kade New Media | İstanbul<br/>thekademedia@gmail.com</p>
            </div>
          </div>
        `,
      });
