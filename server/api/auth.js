...
async function handleLogin(req, res) {
  ...
  try {
    ...
    if (!user) {
      await logActivity({ action: 'Başarısız giriş denemesi', detail: `Bilinmeyen kullanıcı adı: ${username}`, type: 'security', icon: '⚠️', user: 'sistem' }).catch(() => {});
    ...
    if (!valid) {
      await logActivity({ action: 'Başarısız giriş denemesi', detail: `Yanlış şifre: ${user.username}`, type: 'security', icon: '⚠️', user: 'sistem', targetType: 'user', targetId: user.id }).catch(() => {});
      return res.status(401).json({ error: 'Geçersiz kullanıcı adı veya şifre' });
    }
    ...
    await logActivity({ action: 'Admin girişi yapıldı', detail: `${user.username} giriş yaptı`, type: 'system', icon: '🔐', user: user.username, targetType: 'user', targetId: user.id }).catch(() => {});
