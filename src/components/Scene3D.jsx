    ...
    <group ref={meshRef}>
      {particles.map((particle, i) => (
        <mesh key={i} position={particle.position}>
          ...
          <meshBasicMaterial color="#eac321" transparent opacity={0.5} />
        </mesh>
      ))}
    </group>
  )
    ...
    <div ref={containerRef} style={{ position: 'absolute', inset: 0, ...style }} className={className}>
      {isVisible && (
        ...
        >
          ...
          <directionalLight position={[5, 5, 5]} intensity={0.8} color="#eac321" />
          <pointLight position={[-5, -5, 5]} intensity={0.5} color="#eac321" />
          
          <GlowingSphere position={[3, 1, 0]} scale={1.2} speed={0.5} color="#eac321" />
          <GlowingSphere position={[-3.5, -1.5, -2]} scale={0.8} speed={0.7} color="#eac321" />
