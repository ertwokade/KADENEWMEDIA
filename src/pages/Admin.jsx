              ...
              <tbody>
                {visible.map((project, index) => (
                  <tr key={project.slug}>
                    ...
                    <td>{PROJECT_KINDS[project.kind] || '—'}</td>
                    <td>{project.year || '—'}</td>
                    <td>
                      <button
                        type="button"
            ...
            >
              ...
              <div className="admin-form" style={{ border: 'none', padding: 0 }}>
                ...
                <div className="form-row">
                  ...
                  <div className="form-group">
                    <label htmlFor="pf-kind">Kart türü</label>
                    <select id="pf-kind" value={form.kind}
                      onChange={(e) => setForm({ ...form, kind: e.target.value })}>
                      {Object.entries(PROJECT_KINDS).map(([value, label]) => (
                        <option key={value} value={value}>{label}</option>
                      ))}
                    </select>
                    <small className="form-hint">
                      “Müşteri projesi” yalnızca yayın izni ve gerçek medya varsa seçilmelidir.
                      Diğer türler kartın üstünde etiket olarak görünür.
                    </small>
                  </div>
                </div>
...
function emptyProjectForm() {
  return {
    ...
    // Varsayılan tür "müşteri projesi" DEĞİL: izin/medya olmadan hiçbir kayıt
    // müşteri işi gibi yayınlanmasın.
    kind: 'capability',
