...
Kullanıcı talimatı, kök `vercel.json` içinde bir `"services"` alanı üzerinden **tek Vercel projesinde** `legacy` ve `kadexai` adında iki servisin çalıştığını, bunun `kadirdemirs-patch-3` branch'inde hazırlanıp `PR #8` ile açıldığını varsayıyordu. Bu oturumda kodu ve Vercel'in resmî dokümantasyonunu doğrudan kontrol ettim:
...
Kod ve `apps/kadexai/DEPLOYMENT.md` incelemesi, kullanıcı talimatındaki anlatının aksine, **bu sorunun daha önceki bir oturumda zaten çözüldüğünü ve doğrulandığını** gösteriyor:

- `apps/kadexai/DEPLOYMENT.md` (satır 5, 7, 15): "İki proje aynı repository'yi kullanır; `kadenewmedia.com` domaini yalnız ana site projesinde kalır... Doğrulanan Vercel düzeni iki ayrı projedir: ana domain `kademedia`, KadexAI upstream `kade-social-media-ai` (`kadexai.vercel.app`)... Ana domain deployment'ı `dpl_3oPjzNezsbf4Zx173qviGebzTaYa`." Belgeye göre `/kadexai/login`, `/kadexai/dashboard`, `/kadexai/api/health` production'da bu düzenle **doğrulanmış.**
- Kök `vercel.json`: `/kadexai` ve `/kadexai/:path*` → `https://kadexai.vercel.app/kadexai` rewrite (dosyada mevcut, bu oturumda değiştirilmedi).
- `apps/kadexai/vercel.json`: `framework: nextjs`, kendi bağımsız build/install komutları — ayrı bir proje olarak deploy edilecek şekilde doğru yapılandırılmış.
- `apps/kadexai/next.config.ts`: `basePath: '/kadexai'` doğru ayarlanmış; CSP, HSTS, COOP/CORP, X-Frame-Options gibi güvenlik başlıkları zaten mevcut.
- `apps/kadexai/lib/appConfig.ts` + `lib/supabase/cookieOptions.ts`: `withBasePath()`/`stripBasePath()` yardımcıları ve Supabase session cookie'si `path: '/kadexai'` ile doğru scope edilmiş; `domain` alanı kasıtlı olarak boş bırakılmış (rewrite şeffaf olduğundan tarayıcı zaten `kadenewmedia.com` görür). Auth callback route'u (`app/auth/callback/route.ts`) yönlendirmeleri hep `withBasePath()` üzerinden yapıyor, kökten (`/login`) sızıntı yok.
...
- Bu dalın `vercel.json`'ı **gerçekten** `"services":{"legacy":{...},"kadexai":{...}}` yapısını ve `"destination":{"service":"kadexai"}` şeklinde rewrite hedefleri kullanıyor — yani kullanıcı talimatındaki anlatı doğru bir GitHub dalına dayanıyordu.
...
- `/kadexai/api/payments/admin/pricing` endpoint'i bu dalda da mevcut — ama bu, dala özgü bir ekleme değil, zaten `main`'de PR #6 ile önceden var olan `pricingConfig.ts`/route.ts dosyalarının aynısı (bkz. yukarıdaki bölümler).
...
2. `apps/kadexai/lib/payments/pricingConfig.ts`, `apps/kadexai/lib/supabase/admin.ts` — `origin/main`'in kendi test altyapısında var olan bir `server-only` modül çözümleme kusuru düzeltildi (detay: `TEST_REPORT_TR.md`).
3. `.gitignore` — sandbox'a özgü geçici dosya desenleri eklendi.

`vercel.json` (ne kökte ne `apps/kadexai`'de) **değiştirilmedi** — zaten doğru ve production'da doğrulanmış durumda; `kadirdemirs-patch-3`'teki geçersiz "services" şemasına geçirmek mevcut, çalışan yapılandırmayı bozardı.
...
| `apps/kadexai` `npm run typecheck` | Başarılı |
| `apps/kadexai` `npm run lint`, `test:unit` | Başarılı (0 hata, 22/22 test) |
| `apps/kadexai` `npm run build` | Bu sandbox'ta Google Fonts ağ erişimi olmadığı için başarısız — kod hatası değil, ortam kısıtı |
...
| Eski ayrı `kadexai` Vercel projesinin custom domain/webhook/cron bağımlılıkları | **İncelenemedi** — Vercel dashboard erişimi yok |
