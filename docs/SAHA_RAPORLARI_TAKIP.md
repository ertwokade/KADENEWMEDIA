...
- **KadexAI #01:** Canlıda 12 fikrin 8'i şablonda kalıyordu. 12 fikir tek istek/4000 token ve uzun UUID ile gidiyordu; cevap kesilince veya kimlik bozulunca AI çıktısı eşleşmiyordu. Fikirler 2'şerli paralel gruplar (sağlayıcının 25 sn sınırının altında) ve kısa anahtarlarla işleniyor; bir grubun hatası diğerlerini silmiyor, API kaç fikrin AI ile üretildiğini ve hata türünü bildiriyor.
...
- **KadexAI #24/#53 Video Fabrikası (canlı denendi):** gömülü motorun import ettiği `toml`, `loguru`, `requests`, `openai`, `google-genai` imajda yoktu; her istek "bağımlılıklar kurulu değil" dönüyor ve kullanıcıya `pip install` komutu gösteriyordu. Paketler eklendi, medya imajı derlenirken motor importu doğrulanıyor. Canlıda motor yükleniyor; üretim artık yalnız eksik `PEXELS_API_KEY`/`PIXABAY_API_KEY` nedeniyle açık bir mesajla duruyor (Ayarlar → Altyapı'da görünür).
- **KadexAI #53 AI Thumbnail (canlı denendi):** genel "Gemini görsel isteği tamamlanamadı" hatası sınıflandırıldı; emekli görsel modeli otomatik yeniden seçiliyor. Canlı gerçek neden: Gemini anahtarı görsel üretiminde **429 kota** döndürüyor — Google AI Studio'da faturalandırma/kota gerekli.
- **16 Eylül canlı doğrulama:** İçerik Fikirleri 12/12 AI (şablon 0, hata 0); admin AI 5 başlığın tamamı; Operasyon Merkezi örnek verisi temizlendi (₺0); Materyal ekranı güncel deneme ve nedeni gösteriyor; statik sayfalarda onaysız ölçüm isteği yok, GA ölçüm kimliği geçerli.
...
| No | Rapordaki bulgu | Durum / kalan doğrulama |
| --- | --- | --- |
| 01 | Fikir üretimi yapay zekâ kullanmıyor, şablon dolduruyor | 16 Eylül canlı: 12 fikrin 8'i şablondaydı. Tek büyük istek yerine 2'şerli paralel gruplar ve kısa kimliklerle düzeltildi; grup hatası diğerlerini silmiyor. |
