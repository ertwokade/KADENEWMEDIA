...
| REQ-CODE-010 | §30 Otomatik test senaryoları (ticaret/yetki/route/mobil/video/@link) | Kısmen tamamlandı | Playwright zaten bağımlıydı ama hiç yapılandırılmamıştı (`playwright.config.js` yoktu). Bu turda kuruldu: `tests/e2e/routes.spec.js` (19 public route smoke test + 404 + blog-detail regresyon testi), `auth-boundary.spec.js` (oturumsuz `/admin` girişe düşüyor mu), `mobile-chromium` projesi + `mobile.spec.js` (6 kritik sayfada yatay taşma kontrolü — hepsi geçti). `npm run legacy:test:e2e` ile çalıştırılır. **Gerçek bir bulgu ortaya çıkardı:** `/` React uygulaması değil, `public/site.html` (ayrı bir Next.js derlemesinden vendored statik export) üzerinden serviste — konsola "Minified React error #418" (hydration uyuşmazlığı) basıyor, kaynak bu repoda olmadığı için güvenle yamanamadı, testte bilinçli/belgeli olarak muaf tutuldu. Ticaret/ödeme akışı ve video/@link senaryoları canlı kredensiyal gerektirdiği için eklenmedi |
