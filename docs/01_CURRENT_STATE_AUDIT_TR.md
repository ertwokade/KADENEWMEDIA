...
- **Ana uygulama(lar):** Üç ayrı uygulama tek repoda: (1) kök — Vite 8 + React 19 SPA + Express 5 API (`server.js`, `api/[...path].js`), (2) `apps/kadexai` — Next.js 16 App Router, (3) `apps/studio-web`/`apps/studio-worker` — ayrı bir video-editör ürünü ("Kade Studio", bu şartnamenin kapsamı dışında görünüyor, ayrıca değerlendirilmeli).
...
- **Kullanıcı paneli rotası:** `/musteri-panel` (kök, `CustomerPortal.jsx`) + `apps/kadexai/app/dashboard/**` (ayrı, kapsamlı bir dashboard — 30+ alt sayfa).
- **Mevcut ödeme sağlayıcısı:** Shopier (hem kök `server/api/shopier.js` hem `apps/kadexai/lib/payments/shopierProvider.ts` — iki bağımsız entegrasyon).
- **Auth sağlayıcısı:** Kök: özel bcrypt+JWT (admin + müşteri, iki ayrı cookie: `kade_admin_session`, `kade_customer_session`). `apps/kadexai`: Supabase Auth (`@supabase/ssr`).
- **Veritabanı/ORM:** Her ikisi de **Supabase (Postgres)**, ORM yok (doğrudan `@supabase/supabase-js` client). Kök uygulama bu oturumda MongoDB'den Supabase'e taşındı (bkz. bu konuşmanın önceki turları, commit `0a722cb`) — **aynı Supabase projesi** `apps/kadexai` ile paylaşılıyor, tablo adları `kade_*` önekiyle ayrıştırılmış.
...
- **Deployment:** Vercel, iki ayrı proje (kök `kademedia` + `kadexai.vercel.app`), `vercel.json` rewrite ile `/kadexai/**` ikinci projeye yönlendiriliyor. **Bu mimari önceki oturumda Vercel'in resmî dokümantasyonundan doğrulandı ve doğru bulundu** — `vercel.json`'da "services" adlı tek-proje alanı yok, `kadirdemirs-patch-3`/PR #8 dalındaki aksi yöndeki değişiklik geçersiz şema içeriyor, merge edilmemesi öneriliyor (bkz. `VERCEL_SERVICES_MIGRATION_REPORT_TR.md`).
...
- **Mevcut paket/fiyat veri kaynağı:** Kök sitede karışık: `/paketler` sayfası statik `src/data/packages.js` (3 kapsam, fiyatsız) + admin'den girilen fiyat override'ı (bu oturumda wire edildi, commit `9c2a6aa`). `apps/kadexai` tarafında gerçek DB-backed fiyatlandırma var: `lib/payments/pricingConfig.ts` + `kadexai_pricing_overrides` tablosu + admin API (`/api/payments/admin/pricing`).
...
| `apps/kadexai` framework | Next.js 16 (App Router, Turbopack) |
...
| Test altyapısı | Kök: `node --test` (native Node test runner, 15 test). `apps/kadexai`: aynı desen (`tsx --test`, 23 test). E2E altyapısı **yok** (Playwright kurulu ama proje testleri için bir E2E suite tespit edilmedi). |
...
| Analytics | GA4 (`gtag`, `VITE_GA_ID` fallback ile hardcoded bir ID var — **hardcoded fallback ID kod incelemesi gerektirir**), kendi first-party pageview/traffic-source/ai-usage tabloları (`kade_pageviews`, `kade_traffic_sources`, `kadexai_usage`). `apps/kadexai`'de ayrıca Sentry + PostHog. |
| AI sağlayıcıları | Kök: yalnızca Gemini (`server/api/chat.js`, model adı kodda sabit — şartname §26/Faz'daki "model adını merkezi config'e bağla" kuralına aykırı, **bulgu**). `apps/kadexai`: çoklu sağlayıcı, merkezi `lib/ai/provider.ts` üzerinden (`claude`/`gpt4o`/`gemini`), CLAUDE.md'de bu zorunlu kural olarak yazılı. |
...
| `apps/kadexai` `npm run lint` | **0 hata** |
| `apps/kadexai` `npm run typecheck` | **0 hata** |
| `apps/kadexai` `npm run test:unit` | **23/23 geçti** (RLS migration, idempotency, IDOR, AI kota, ödeme imza doğrulama testleri dahil) |
| `apps/kadexai` `npm run build` | **Başarılı** — önceki denetim turunda ağ kısıtı (Google Fonts) nedeniyle doğrulanamamıştı, bu oturumda ağ erişimi vardı ve build tamamlandı. |
...
| Aktivite Logu ve Bildirim çan menüsü zaman damgası bozuktu | **Düzeltildi** | `server/api/notifications.js` | Aynı kalıp: `action=activity` ve genel bildirim GET'i ham `id`/`created_at` dönüyordu, `Admin.jsx` `log._id`/`log.createdAt`/`n._id`/`n.createdAt` okuyordu — Aktivite Logu'nda her satır "NaN dk önce", bildirim menüsünde boş zaman gösteriyordu. `mapActivityLog()`/`mapNotification()` eklendi |
| `logActivity()`'nin iki senkron olmayan kopyası vardı | **Düzeltildi** | `server/api/notifications.js` ↔ `server/api/_lib/notify.js` | `_lib/notify.js`'teki fonksiyon hiçbir yerden import edilmiyordu (ölü kod) — tüm ~40 çağrı noktası `notifications.js` içindeki ayrı bir kopyayı kullanıyordu. Bu, Faz 3'te `_lib/notify.js`'e eklenen audit-alan genişletmesinin hiçbir yerde etkili olmamasına yol açmıştı. `notifications.js` artık `_lib/notify.js`'i tek kaynak olarak import edip yeniden dışa aktarıyor |
...
| İki bağımsız Shopier entegrasyonu | Orta | kök + `apps/kadexai` | Kod tekrarı + iki ayrı webhook secret yönetimi riski, birleştirme değerlendirilmeli |
...
- `apps/kadexai` Shopier entegrasyonu: ayrı, kendi imza doğrulaması ve idempotency testleri var (23 testin bir kısmı bunu kapsıyor).
- **Her iki tarafta da entitlement/yetkilendirme backend seviyesinde uygulanıyor** (frontend-only kontrol değil) — `requirePermission`/`requireAdmin` (kök), RLS + `requirePermission` benzeri kontroller (kadexai).
...
1. **Üç ürün, tek repo:** Kade New Media (ajans+ticaret), KadexAI (apps/kadexai, tool platformu), Kade Studio (apps/studio-web/worker, video editör). Şartname §17 yeni bir "ChatCut alternatifi" video editör istiyor — **Kade Studio'nun bu ihtiyacı zaten karşılayıp karşılamadığı ilk incelenmesi gereken soru**, aksi halde dördüncü bir video editör inşa edilmiş olur.
