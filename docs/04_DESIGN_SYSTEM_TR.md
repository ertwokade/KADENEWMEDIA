...
## 2. `apps/kadeai` tokenları — karşılaştırma

Kaynak: `apps/kadeai/app/globals.css`.
...
- Nötr skala: Tailwind `zinc-50…zinc-950` (CLAUDE.md'de de dokümante). Kök sitenin
  ...
  **Karar: Bu bilinçli bir ayrım olarak korunmalı** — kadeai bir "araç paneli"
...
| Bileşen | Kök'te var mı | kadeai'de var mı | Durum |
|---|---|---|---|
| Layout/Header/Footer | `src/components/Navbar.jsx`, `Footer.jsx` | `apps/kadeai/components/layout/*` (CLAUDE.md'de referans var) | Ayrı implementasyonlar, paylaşılmıyor — **beklenen** (ayrı Next.js/Vite projeleri, kod paylaşımı build-sistemi karmaşıklığı doğurur) |
...
| Modal | `admin-modal-overlay`/`admin-modal` (Admin.jsx içinde) | — | Kökte var, kadeai'de doğrulanmadı |
