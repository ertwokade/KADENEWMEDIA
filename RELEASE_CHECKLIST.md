...
- [x] KadeAI `npm run lint`
- [x] KadeAI `npm run typecheck`
- [x] KadeAI `npm run test:unit` — 7/7
- [x] KadeAI `npm run build` — 41 static page üretimi tamamlandı
- [x] Root ve KadeAI `npm audit --omit=dev --audit-level=moderate` — 0 vulnerability
- [ ] **BLOCKER:** Uygulama içi tarayıcı bağlanarak Playwright E2E, console, responsive viewport ve axe testi çalıştırılsın.
- [ ] **BLOCKER:** `python3` ortamına backend requirements + pytest kurularak `apps/kadeai/backend/tests` çalıştırılsın.
...
- [ ] `NEXT_PUBLIC_BASE_PATH=/kadeai`
- [ ] `NEXT_PUBLIC_APP_URL=https://kadenewmedia.com/kadeai`
...
### KadeAI payment — varsayılan kapalı
...
- [ ] `apps/kadeai/supabase/migrations/202607170001_security_hardening.sql` gözden geçirildi/uygulandı.
- [ ] `apps/kadeai/supabase/migrations/202607170002_explicit_rls_and_payments.sql` staging’de uygulandı.
...
- [ ] Supabase cookie scope `/kadeai` deployment davranışıyla uyumlu.
- [ ] Vercel `/kadeai/:path*` rewrite hedefi doğru ve base path iki kez eklenmiyor.
...
- [ ] `/admin`, `/musteri-panel`, KadeAI dashboard ve tüm API response’larında no-store doğrulandı.
...
4. Payment provider kapalıyken ana site + KadeAI health/auth/data akışlarını doğrula.
5. Production deploy et; `/`, `/giris`, `/admin`, `/kadeai/login`, private dashboard redirect ve health endpoint’lerini kontrol et.
...
- [x] KadeAI lint, type-check, 10 unit test ve production build.
...
- [ ] Staging admin/customer/KadeAI rol fixture'ları — BLOCKED_BY_ENVIRONMENT.
