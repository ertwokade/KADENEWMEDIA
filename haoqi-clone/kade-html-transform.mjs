...
/* Inline so the first painted pixel is intentional even on a cold cache. It
   uses the site's grid and star language, contains no text and no progress
   bar, and fades as soon as the real scene is ready. */
const CRITICAL_FIRST_PAINT = `<style data-kade-critical-first-paint>@keyframes kade-boot-star{0%,100%{transform:translate(-50%,-50%) rotate(-7deg) scale(.94)}50%{transform:translate(-50%,-50%) rotate(7deg) scale(1.04)}}html:not([data-kade-loaded]) body::before{content:"";position:fixed;inset:0;z-index:2147483000;background-color:#fbfaf4;background-image:linear-gradient(rgba(54,54,48,.1) 1px,transparent 1px),linear-gradient(90deg,rgba(54,54,48,.1) 1px,transparent 1px);background-size:33.333vw 33.333vh;pointer-events:none}html:not([data-kade-loaded]) body::after{content:"";position:fixed;left:50%;top:50%;z-index:2147483001;width:clamp(68px,7vw,104px);aspect-ratio:1;background:radial-gradient(ellipse at 39% 49%,#111 0 6%,transparent 7%),radial-gradient(ellipse at 61% 49%,#111 0 6%,transparent 7%),#ffd92f;clip-path:polygon(50% 0,61% 34%,98% 36%,68% 57%,78% 94%,50% 72%,22% 94%,32% 57%,2% 36%,39% 34%);animation:kade-boot-star 1.15s ease-in-out infinite;pointer-events:none}html.dark:not([data-kade-loaded]) body::before{background-color:#000;background-image:linear-gradient(rgba(251,250,244,.14) 1px,transparent 1px),linear-gradient(90deg,rgba(251,250,244,.14) 1px,transparent 1px)}html[data-kade-loaded] body::before,html[data-kade-loaded] body::after{opacity:0;visibility:hidden;transition:opacity .22s ease,visibility 0s linear .22s}</style>`
