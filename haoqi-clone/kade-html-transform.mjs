...
const COPY = [
  ...
  /* Sahnedeki footer üç bağlantıyla geliyordu; altı hesabın tamamı burada
     gösterilsin diye üç bağlantı daha ekleniyor. Sunucu HTML'i ile bundle
...
export const SCRIPT_COPY = [
  ...
  ['{href:"https://github.com/wenhaoqiasd",label:"GitHub"}', '{href:"https://www.youtube.com/@kadenewmedia",label:"YouTube"},{href:"https://x.com/kadenewmedia",label:"X"},{href:"https://www.linkedin.com/company/kadenewmedia",label:"LinkedIn"},{href:"https://wa.me/905067293423",label:"WhatsApp"}'],
  ['className:"flex flex-row items-center gap-2 lg:gap-4"', 'className:"flex flex-row flex-wrap items-center gap-2 lg:gap-4"'],
  ['"/reunimos"', '"/referanslar"'],
  ['"/inspire_mono"', '"/hizmetler/sosyal-medya-yonetimi"'],
  ['"/wasm_design_utils"', '"/hizmetler/strateji-danismanlik"'],
  ['"/adrive"', '"/hizmetler/reklam-yonetimi"'],
...
const HOMEPAGE_NOJS = '<noscript data-kade-nojs><style>html::before,body::before,body::after{display:none!important}.invisible{visibility:visible!important}</style></noscript>'
...
export function transformHtml(html) {
  ...
  if (isHomepageSnapshot) {
    ...
    if (!html.includes('data-kade-nojs')) html = html.replace('</head>', HOMEPAGE_NOJS + '</head>')
