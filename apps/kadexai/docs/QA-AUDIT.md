# KadexAI — Full-stack ve QA denetimi

Son doğrulama: 16 Temmuz 2026  
Üretim yolu: `https://kadenewmedia.com/kadexai`
...
- `/kadexai` base path; istemci istekleri, statik dosyalar, iframe, manifest, service worker, auth callback ve parola sıfırlama boyunca korundu.
...
Migration: `supabase/migrations/202607160001_kadexai_profiles_and_runs.sql`
...
- `/kadexai` ve `/kadexai/` açılışı
- `/kadexai/login`, parola sıfırlama ve manifest
- Korumalı derin route’un base path’i iki kez eklemeden login’e dönmesi
- Operations iframe route’unun oturum koruması
- `/kadexai` dışındaki route’un uygulama tarafından sahiplenilmemesi
...
Kod ve migration hazırdır; ancak uzak Supabase projesine migration repository’den otomatik uygulanmaz. Deployment öncesi SQL Editor’da migration çalıştırılmalı, production environment değerleri girilmeli ve Supabase Auth redirect allow-list’e `https://kadenewmedia.com/kadexai/auth/callback` ile `https://kadenewmedia.com/kadexai/reset-password` eklenmelidir. AI ve Shopier anahtarları olmadan ilgili özellikler açık “yapılandırılmadı” durumu gösterir; sahte başarı üretmez.

Nginx snippet’i `deploy/nginx-kadexai.conf`, çalıştırılabilir kurulum ve rollback adımları `deploy/README.md` içindedir. Snippet yalnızca `/kadexai` konumunu proxy eder ve ana sitenin diğer route’larına dokunmaz.
