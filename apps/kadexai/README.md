...
KadexAI, Next.js 16 App Router ve React 19 ile geliştirilmiş içerik/operasyon panelidir. Web uygulamasının tamamı `/kadexai` base path altında çalışır; auth ve veri katmanı Supabase, ayrı medya servisi FastAPI kullanır.
...
Login: `http://127.0.0.1:3000/kadexai/login`. `KADE_DISABLE_AUTH=1` yalnız development ortamında geçerlidir ve production’da yok sayılır.
...
2. `supabase/migrations/202607160001_kadexai_profiles_and_runs.sql`
...
```powershell
curl.exe -X POST https://kadenewmedia.com/kadexai/api/generate/title `
