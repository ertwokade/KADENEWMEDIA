 ...
 *
 * Not: Musterinin zaten bir KadexAI hesabi olmasi gerekir (odeme sirasinda
...
export async function createDynamicOffer(input: CreateDynamicOfferInput): Promise<DynamicOfferResult> {
  ...
  if (!matchedUser) {
    throw new Error('Bu e-posta ile kayitli bir KadexAI hesabi bulunamadi. Musteri once hesap olusturmali, sonra tekrar deneyin.')
  }
  ...
  const appUrl = (process.env.NEXT_PUBLIC_APP_URL || 'https://kadenewmedia.com/kadexai').replace(/\/$/, '')
