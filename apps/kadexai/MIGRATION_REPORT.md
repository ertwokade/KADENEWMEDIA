...
- Dal: `security-hardening-kadexai`
...
- Ana domain kaynak yedeğinde `/kadexai` için dar Vercel rewrite hazırlandı ve build/lint geçen `integrate-kadexai-route` / `f106176` teslim noktası oluşturuldu.
...
- KadexAI klasörünün yerel Vercel bağlantısı yanlış ana site projesinden ayrılarak `kade-social-media-ai` projesine düzeltildi.
...
- Supabase Site URL/redirect allow-list production `/kadexai` değerleriyle Management API üzerinden güncellendi ve doğrulandı.
- KadexAI production upstream ve ana domain rewrite deployment’ları READY; ana `/` korunarak `/kadexai` smoke ve gerçek auth akışı geçti.
