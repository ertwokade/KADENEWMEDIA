...
import { assignPlanDates, istanbulToday, planSlots } from '@/lib/contentPlan'
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const slots = planSlots(frequency || 'haftada 3', start)
    const result = await generateContent({
      prompt: buildContentPlanPrompt(niche, platform, goal || 'takipçi büyümesi', frequency || 'haftada 3', slots),
    ...
    const rawDays = asRecordList(parsed?.gunler, (day) => {
    ...
    const days = assignPlanDates(rawDays, slots)
