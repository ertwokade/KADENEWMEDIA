...
import { sendMorningOperationBriefing } from '@/lib/notifications/operationFeed'
...
/** Gün başı brifingini WhatsApp'a gönderir. Cron veya sahip tetikler. */
export async function GET(request: NextRequest) {
  ...
  try {
    return NextResponse.json(await sendMorningOperationBriefing(), { headers })
  } catch (error) {
    captureApiError(error, '/api/notifications/morning-briefing')
    ...
    return NextResponse.json({
      error: 'Gün başı brifingi gönderilemedi.',
