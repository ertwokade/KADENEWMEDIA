  ...
  try {
    ...
    if (action === 'login') {
      ...
      if (error) {
        ...
        console.error('[kadexai/login] signIn reddedildi:', {
        ...
        return NextResponse.json({
          error: 'Kullanıcı adı/e-posta veya parola hatalı. Admin panelindeki hesabını ya da mevcut KadexAI hesabını kullanabilirsin.',
        }, { status: 401, headers })
      }
    ...
    if (error) {
      console.error('[kadexai/signup] kayıt reddedildi:', {
        code: (error as { code?: string }).code,
        status: error.status,
        message: error.message,
      })
