...
OUTPUT = ROOT / "public" / "kadexai" / "downloads" / "kadexai-sirket-briefi-ornek.docx"
...
def build_document():
    ...
    left = header.add_run("KadexAI  /  ŞİRKET HAFIZASI")
    ...
    kicker_run = kicker.add_run("KadexAI şirket hafızası şablonu")
    ...
    subtitle_run = subtitle.add_run(
        "KadexAI'nin markanızı, hedef kitlenizi ve içerik kurallarınızı tanıması için düzenlenebilir örnek."
    )
    ...
    note_run = note.add_run(
        "Kullanım: Bu örnekteki bilgileri kendi şirketinizle değiştirin. Başlıkları koruyun ve dosyayı KadexAI > Ayarlar > Şirket Briefi alanından yükleyin."
    )
    ...
    closing_run = closing.add_run(
        "İpucu: Briefiniz ne kadar açık ve güncelse, KadexAI'nin ürettiği içerik de markanıza o kadar tutarlı yaklaşır."
    )
    set_run_font(closing_run, 10, MUTED, italic=True)

    document.core_properties.title = "KadexAI Şirket Briefi Örneği"
    document.core_properties.subject = "KadexAI şirket hafızası için örnek marka briefi"
    document.core_properties.author = "Kade New Media"
    document.core_properties.keywords = "KadexAI, şirket briefi, marka hafızası, içerik"
