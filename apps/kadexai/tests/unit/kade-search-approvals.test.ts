...
const idea = sanitizeApprovalIdea({
  ...
  cta: 'Kontrol listesini kaydet', hashtagler: ['kadexai', '#icerik'], paylasimSaati: ['19:00'],
...
test('onay fikri temizlenir ve üretim paketine dönüşür', () => {
  ...
  assert.deepEqual(draft.hashtags, ['#kadexai', '#icerik'])
...
test('onaylı içerik WhatsApp mesajı güvenli ve eyleme dönüktür', () => {
  const message = formatApprovalWhatsApp(idea, buildApprovalDraft(idea), 'https://kadenewmedia.com/kadexai/dashboard/kade-search')
  assert.match(message, /Onaylı İçerik Paketi/)
  assert.match(message, /#kadexai/)
...
test('haftalık rapor tarih anahtarı ve hata özeti üretir', () => {
  ...
  const message = formatWeeklySiteReport(audit, 'https://kadenewmedia.com/kadexai/dashboard')
