...
test('BYOK çağrısının maliyeti KadexAI hesabına yazılmaz', () => {
...
test('token kotası varsayılan olarak zorlanmaz', () => {
  const original = process.env.KADEXAI_ENFORCE_TOKEN_QUOTA
  try {
    delete process.env.KADEXAI_ENFORCE_TOKEN_QUOTA
    assert.equal(isTokenQuotaEnforced(), false)
    process.env.KADEXAI_ENFORCE_TOKEN_QUOTA = '1'
    assert.equal(isTokenQuotaEnforced(), true)
  } finally {
    if (original === undefined) delete process.env.KADEXAI_ENFORCE_TOKEN_QUOTA
    else process.env.KADEXAI_ENFORCE_TOKEN_QUOTA = original
  }
})
