...
Masaüstündeki **KADE SEARCH** motorunun kadeai'ye taşınmış hâli. Yapay zekâ
...
curl -X POST -H "x-cron-secret: $CRON_SECRET" -H 'content-type: application/json' \
  ...
  https://kadenewmedia.com/kadeai/api/kade-search/collect
# ...diğer kaynaklar... sonra:
curl -X POST -H "x-cron-secret: $CRON_SECRET" -H 'content-type: application/json' \
  -d '{"finalize":true}' https://kadenewmedia.com/kadeai/api/kade-search/collect
```
...
2. Yetkili yönlendirme URI'si: `<NEXT_PUBLIC_APP_URL>/api/youtube/callback`
   (üretimde `https://kadenewmedia.com/kadeai/api/youtube/callback`).
