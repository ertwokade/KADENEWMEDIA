...
try {
  ...
  if (remoteBaseUrl) {
    ...
    const remoteLogin = await fetch(`${remoteBaseUrl}/kadeai/api/auth/password`, {
      method: 'POST',
  ...
  const appEnv = {
    ...
    NEXT_PUBLIC_APP_URL: 'http://127.0.0.1:3100/kadeai',
    PORT: '3100',
    HOSTNAME: '127.0.0.1',
  }
  ...
  if (!remoteBaseUrl) {
    ...
    await waitForHealth('http://127.0.0.1:3100/kadeai/api/health')
  }
