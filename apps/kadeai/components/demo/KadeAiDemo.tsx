...
const DAILY_LIMIT = 3
const STORAGE_KEY = 'kadeai-public-demo-v1'

function todayKey() { return new Date().toISOString().slice(0, 10) }

export default function KadeAiDemo({ content }: { content: DemoPageContent }) {
  ...
  function runDemo() {
    ...
    if (record.count >= DAILY_LIMIT) {
      setError('Bugünkü 3 demo hakkın tamamlandı. KadeAI hesabında tam araçları kullanabilirsin.')
      return
    }
    record.count += 1
  ...
  return (
    <main className="min-h-screen bg-[#0c0c0d] text-zinc-100">
      <nav className="border-b border-white/10"><div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4"><Link href="/" className="text-sm font-black tracking-[0.18em] text-[#f2c322]">KADE AI</Link><div className="flex items-center gap-4 text-xs"><Link href="/paketler" className="text-zinc-400 hover:text-white">Planlar</Link><Link href="/kadeai/login" className="rounded-full bg-[#f2c322] px-4 py-2 font-bold text-zinc-950">KadeAI’yi Dene</Link></div></div></nav>
