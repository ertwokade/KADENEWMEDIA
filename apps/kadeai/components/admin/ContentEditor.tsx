...
export default function ContentEditor() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const response = await apiFetch('/api/admin/content?key=kadeai-demo', { cache: 'no-store' })
  ...
  async function save() {
    ...
    setMessage('')
    try {
      const response = await apiFetch('/api/admin/content?key=kadeai-demo', {
        method: 'PUT',
  ...
  return (
    <div className="space-y-5">
      ...
      <p className="rounded-lg border border-zinc-800 bg-zinc-900/60 p-3 text-xs leading-5 text-zinc-400">
        Bu metinler <code className="font-mono text-zinc-300">/kadeai-demo</code> sayfasını besler.
