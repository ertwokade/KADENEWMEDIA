# KadeAI env değişkenleri — `kadeertwo/kadenewmedia` projesine girilecek
...
`apps/kadeai/scripts/validate-env.mjs` bunları zorunlu tutuyor:
...
`KADEAI_ADMIN_API_SECRET` · `KADE_TOKEN_ENCRYPTION_KEY` (BYOK şifrelemesi — kaybolursa
...
- `NEXT_PUBLIC_BASE_PATH` — varsayılan `/kadeai` zaten doğru, elle girilirse bozulur.
...
| `KADEAI_ENFORCE_TOKEN_QUOTA` | Kota ölçülür ama kimse kesilmez (önerilen: şimdilik girme) |
