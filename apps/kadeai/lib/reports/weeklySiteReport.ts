...
const CRITICAL_PATHS = ['/', '/hizmetler', '/portfolio', '/basari-hikayeleri', '/blog', '/iletisim', '/giris/danismanlik', '/kadeai/login']
...
async function fetchText(url: string) {
  ...
  try {
    const response = await fetch(url, {
      cache: 'no-store',
      headers: { 'user-agent': 'KadeAI-Site-Audit/1.0' },
...
export function formatWeeklySiteReport(audit: WeeklySiteAudit, dashboardUrl: string) {
  ...
  const lines = [
    '*KadeAI · Haftalık Site Raporu*', date, '',
