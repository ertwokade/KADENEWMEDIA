...
- [x] A01 Broken Access Control — root admin API’lerinde DB-backed kullanıcı/rol/permission; KadexAI proxy + handler auth; Supabase sorgularında `user_id`; ayarlar yalnız `thekademedia@gmail.com`.
...
- [x] A04 Insecure Design — payment idempotency, server-side KadexAI product catalog, fail-closed auth config, seed production kapısı.
...
- [x] A06 Vulnerable Components — root ve KadexAI production dependency audit: 0 bilinen vulnerability.
...
- [-] A07 — distributed rate limiting KadexAI’de yok; root’ta production için Upstash yapılandırılması zorunlu.
- [x] A08 Software and Data Integrity Failures — Shopier HMAC, KadexAI raw-body webhook signature ve event idempotency.
...
- [x] API1 BOLA — KadexAI history/templates/calendar/payment sorguları authenticated user ID ile filtreleniyor; RLS ayrıca tanımlı.
...
- [ ] API4 — KadexAI dağıtık rate limit/kredi ledger atomik kota sistemi production ölçeği için eksik.
- [x] API5 Broken Function Level Authorization — root permission/admin matrisi; KadexAI settings owner-only server proxy.
...
- [x] KadexAI mutating API origin kontrolü proxy seviyesinde.
...
- [x] KadexAI settings UI/API yalnız `thekademedia@gmail.com`.
- [x] KadexAI owner/dashboard erişimi yalnız UI gizlemeye dayanmıyor; proxy server-side uygular.
...
- [x] KadexAI transcribe max 25 MiB, MIME + magic signature + timeout.
...
- [x] KadexAI checkout server product catalog kullanır ve idempotency user-bound.
- [x] KadexAI webhook raw body signature ve unique event ID kullanır.
...
- [-] KadexAI bundle secret kontrolü geçti ancak boş ortam nedeniyle 0 yapılandırılmış canary secret taradı.
...
- [ ] KadexAI in-memory rate limiter distributed store’a taşınmalı.
...
- [x] KadexAI dağıtık kota production'da Upstash yoksa 503 fail-closed; minute/day/cost/idempotency atomik Lua akışında.
- [x] Üç canary olmadan bundle testi başarısız; üç sentetik canary root dist, KadexAI static/HTML/RSC çıktılarında bulunmadı.
...
- [x] KadexAI 7 unit test: ownership negatif testi, settings owner, RLS text guard, webhook signature/idempotency, provider/email mock.
- [x] Root ve KadexAI dependency audit: 0 vulnerability.
- [x] Root lint/build ve KadexAI lint/typecheck/build başarılı.
- [ ] KadexAI Playwright E2E/axe: uygulama içi tarayıcı bağlı değil.
